Rcpp::sourceCpp("code/cpp/VVS_Linear_AUX.cpp")
source("code/R/VVS_Linear_AUX.R")
source("code/R/VVS_Linear2_Horseshoe.R")
...
Dataset_4_Bayesreg_1 <- purrr::map(Y1, 
                            ~ as.data.frame(cbind(.x$Y,.x$X)))
Dataset_4_Bayesreg_2 <- purrr::map(Y2, 
                            ~ as.data.frame(cbind(.x$Y,.x$X)))
Dataset_4_Bayesreg_3 <- purrr::map(Y3, 
                            ~ as.data.frame(cbind(.x$Y,.x$X)))
...
for(jj in 1:3){
  ...
  Res_HSmix  <- parallel::mclapply(1:30,FUN = function(x) HSMIX(x,DF_Y = data_Y[[jj]][[x]]), mc.cores = 10, mc.cleanup = T)
  saveRDS(Res_HSmix,paste0("new_simu_HSmix_",jj,".RDS"))
}
