...
data_BR <-
  list(Dataset_4_Bayesreg_1,
       Dataset_4_Bayesreg_2,
       Dataset_4_Bayesreg_3)
...
for (jj in 1:3) {
  ...
  Res_HSplus[[jj]] <-
    parallel::mclapply(
      1:30,
      FUN = function(x)
        bayreg_HSplus(x, DF = data_BR[[jj]][[x]]),
      mc.cores = 15,
      mc.cleanup = T
    )
}
...
for (jj in 1:3) {
  ...
  Res_Ridge[[jj]]  <-
    parallel::mclapply(
      1:30,
      FUN = function(x)
        bayreg_Ridge(x, DF = data_BR[[jj]][[x]]),
      mc.cores = 15,
      mc.cleanup = T
    )
}
